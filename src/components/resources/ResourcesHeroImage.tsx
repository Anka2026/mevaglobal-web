...
import {
  RESOURCES_IMAGE_CHANGED_EVENT,
  RESOURCES_IMAGE_STORAGE_KEY,
...
export function ResourcesHeroImage({ alt = "", imageNote = "", uploadLabel = "" }: Props) {
  ...
  const readStored = useCallback(() => {
    try {
      const s = localStorage.getItem(RESOURCES_IMAGE_STORAGE_KEY);
  ...
  useEffect(() => {
    ...
    window.addEventListener(RESOURCES_IMAGE_CHANGED_EVENT, onChange);
    window.addEventListener("storage", onChange);
    return () => {
      window.removeEventListener(RESOURCES_IMAGE_CHANGED_EVENT, onChange);
  ...
  const onFile = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      try {
        localStorage.setItem(RESOURCES_IMAGE_STORAGE_KEY, url);
      } catch {
        /* quota */
      }
      setDataUrl(url);
      ...
      >
        {displaySrc ? (
          ...
          <div
            ...
            aria-hidden
          >
            <span className="text-sm font-medium text-[color:var(--ink-dark)]/50">—</span>
          </div>
        )}
      </div>
