...
export function VisualAnchor({
  ...
  if (photoPresentation && hasImage) {
    return (
      <div className={cn(imageClip, editorialShadow, "isolate flex min-h-0 min-w-0 flex-1 flex-col", className)}>
        <div className="relative aspect-[4/3] w-full min-h-[200px] max-h-[min(26rem,50vh)] sm:min-h-[220px] sm:max-h-[min(28rem,48vh)] lg:aspect-auto lg:h-full lg:min-h-0 lg:max-h-[min(30rem,46vh)]">
  ...
  return (
    ...
    >
      <div className={cn("relative w-full min-h-0 flex-1", frameHeights)}>
        <div className={visualImageInnerClipClass}>
          {hasImage ? (
            <Image
              src={src!}
